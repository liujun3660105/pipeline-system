...
<script>
  ...
  export default {
    ...
    data(){
      return {
        ...
        dataTypeList:[
          ...
          {value:'shp,shx,dbf',label:'Shp文件',disabled: false, format:'.shp,.shx,.dbf'}
        ],
        coorTypeList:[
    ...
    computed:{
      ...
      workspace(){
        return this.dataType==='dwg'? 'CADeasyTranslatorToGEOJSON.fmw': 'SHPeasyTranslatorToGEOJSON.fmw'
  }
    ...
    methods:{
      ...
      runWorkSpace(){
        ...
        let sourcefilePath = this.dataType === 'dwg' ? this.files[0].path: this.files[1].path;

        // let params = 'SourceDataset_GENERIC'+'='+this.files[0].path+'&'+'SourceFormat'+'='+this.sourceDataType+'&'+'COORDSYS'+'='+this.coorType;
        let CADparams = 'SourceDataset_ACAD'+'='+sourcefilePath+'&'+'SourceFormat'+'='+this.sourceDataType+'&'+'COORDSYS'+'='+this.coorType+'&'+'DestinationFormat'+'='+'GeoJSON';
        let SHPparams = 'SourceDataset_GENERIC'+'='+sourcefilePath+'&'+'SourceFormat'+'='+this.sourceDataType+'&'+'COORDSYS'+'='+this.coorType+'&'+'DestinationFormat'+'='+'GeoJSON';
      ...
      uploadFiles(e){
        ...
        if(e.target.files.length>0){
          ...
          if(fileSize>1*1024*1024){
            this.$Notice.warning({
              title: '文件过大',
              desc: '文件 ' + e.target.files[0].name + ' 文件过大，请上传小于1M的文件'
            });
            //清空文件
            // document.getElementById('file-upload').outerHTML = document.getElementById('file-upload').outerHTML;
            document.getElementById('file-upload').value = '';
          ...
          FMEServer.dataUpload(this.repository, this.workspace, this.fileInput, jsId,  (json)=> {
