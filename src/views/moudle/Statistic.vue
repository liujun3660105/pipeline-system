<template>
    <div class="statistic">
      <Row :gutter="50" class="item-row">
        <i-col span="8" v-for="(infor, i) in inforCardData" :key="`infor-${i}`" style="height: 80px;">
      ...
      <div >
        <Row :getter="50" class="item-row">
          <i-col span="12">
            <Card shadow>
              <ChartPie style="height: 300px;" :pieData="piePcData" :pieInnerData="piePcInnerData"  text="普查数据" ></ChartPie>
            </Card>
          </i-col>
          <i-col span="12">
            <Card shadow>
              <ChartPie style="height: 300px;" :pieData="pieWtData" :pieInnerData="pieWtInnerData" text="物探数据" ></ChartPie>
            </Card>
          </i-col>
        </Row>
        <Row :getter="50" class="item-row">
    ...
    export default {
      ...
      data(){
          return {
            ...
            inforCardData: [
              { title: '普查数据', icon: 'ios-folder', count: 1, color: '#2d8cf0',datatype:'pc' },
              { title: '物探数据', icon: 'md-list-box', count: 1, color: '#19be6b',datatype:'wt' },
              { title: '规划数据', icon: 'ios-create', count: 1, color: '#ff9900',datatype:'xm' },
            ],
            piePcData: [
              { value: 0, name: '给水' },
              { value: 0, name: '中水' },
          ...
          getSumLength(){
            getTotalLength().then(res=>{
              this.inforCardData.forEach((item,index)=>{
                ...
                this.inforCardData[index].count=Math.round(res.data.data[valueIndex].sum/1000);
              })
            })
          },
         searchByPolygon(){
