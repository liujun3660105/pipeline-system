    ...
    <Row :gutter="0" class="item-row">
      ...
      <i-col span="12">
        <div class="upload-item">
          ...
          <div class="upload-info" v-if="file !== null">
            Upload file: {{ file.name }}
            <Button @click="upload" :loading="loadingStatus">{{ loadingStatus ? '正在载入' : '点击上传' }}</Button>
          </div>
        </div>
      </i-col>
    </Row>
...
export default {
  ...
  methods: {
    ...mapMutations("globalFeature", [
      "globalFeaturesChange",
      "DbClickRowChange"
    ]),
    ...mapMutations("collide", ["uploadFeaturesChange"]),
    ...
    upload() {
      ...
      getUploadGeometry(formData).then(res => {
        if (res.status === 201) {
          ...
          this.uploadFeaturesChange(this.uploadGeoJsonStr);
    ...
    selectRow(val, index) {
      console.log(val);
      this.DbClickRowChange(val.xmbm);
    ...
    startAnalyze() {
      ...
      getAnalyzeGeometry(
        ...
        if (this.isJSON(res.data.data)) {
          ...
          let geoJsonResult = JSON.parse(res.data.data);
          console.log("分析后", geoJsonResult);
          this.globalFeaturesChange(geoJsonResult);
          this.gxInfoData = geoJsonResult.features.map(x => x.properties);
        } else {
