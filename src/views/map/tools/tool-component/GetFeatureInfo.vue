<template>
  <div>
    <a @click="isGetFeatureInfo=!isGetFeatureInfo">
      要素查看
      <Icon type="ios-information-circle" size="25" :class="{active:isGetFeatureInfo}"></Icon>
    </a>
    <PopupOverlay
      :position="position"
      :featureInfo="featureInfo"
      :layerType="layerType"
      :isShownAwait="isShownAwait"
    ></PopupOverlay>
  </div>
</template>
  ...
  methods: {
    ...
    getShownWMSLayer() {
      ...
      wmsLayerList.getArray().forEach((wmsLayer, index) => {
        ...
        if (wmsLayer.type === "TILE") {
          ...
          let url =
            wmsLayer.getSource().urls instanceof Array
              ? wmsLayer.getSource().urls[0]
              : "";
    ...
    setLayerIndex(layers, e) {
      ...
      if (this.PromiseTimes < layers.length) {
        let url = layers[this.PromiseTimes]
          .getSource()
